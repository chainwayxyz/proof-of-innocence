...
describe("Test zkp circuit and scripts", function () {
  ...
  it("Should be able to generate input for circuit", async function () {
    const proofStr = await client.calculateProofFromNote(note, (proof) => {});
    console.log('this is our proof string: ', proofStr);
    ...
    // console.log(input);
    const result = await client.verifyProof(input);
    console.log(result);
  });
});
